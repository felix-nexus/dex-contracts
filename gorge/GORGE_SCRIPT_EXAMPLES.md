...
**모든 gorge write 명령어는 다음 구조를 따릅니다:**
...
**토큰 주소와 같이 긴 값만 변수 사용, 숫자는 직접 입력:**

```bash
# ✅ 좋은 예 - 긴 주소는 변수 사용
TOKEN="0x1234567890123456789012345678901234567890"
gorge write ... "createPair(address,uint256,uint256,bytes)" $TOKEN 1000 2000 $FEE_DATA

# ❌ 나쁜 예 - 짧은 숫자까지 변수화
TICK_SIZE="1000"
LOT_SIZE="2000"
gorge write ... "createPair(address,uint256,uint256,bytes)" $TOKEN $TICK_SIZE $LOT_SIZE $FEE_DATA

# ✅ 좋은 예 - 수수료는 직접 입력
gorge write ... "setMarketFees(uint32,uint32,uint32,uint32)" 100 100 50 50

# ❌ 나쁜 예 - 수수료를 변수화
SELLER_MAKER="100"
SELLER_TAKER="100"
gorge write ... "setMarketFees(uint32,uint32,uint32,uint32)" $SELLER_MAKER $SELLER_TAKER ...
```
...
```bash
# 토큰 주소 (긴 주소만 변수 사용)
TOKEN="0x0001000000000000000000000000000000000000"
...
gorge write \
  ...
  0xcb95777d0f8d2EfA5e836Cb65f814dF8C7261d83 \
  0 \
  $(cast calldata "createPair(address,uint256,uint256,bytes)" $TOKEN 1 1 $FEE_DATA)
...
```bash
# 새 FeeCollector 주소
NEW_COLLECTOR="0x9999999999999999999999999999999999999999"

# 1. Game Market (직접 호출)
gorge write \
  --rpc-url https://mainnet.crosstoken.io:22001 \
  --sender 0xafcc9E7d739b03CC53e2152d368f365430CF3CCa \
  --out mainnet_update_fee_collector \
  0xa0f50f79615247530fABcC3efd79B8e5b961b966 \
...
gorge write \
  ...
  0xcb95777d0f8d2EfA5e836Cb65f814dF8C7261d83 \
  0 \
  $(cast calldata "setFeeCollector(address)" $NEW_COLLECTOR)
...
gorge write \
  ...
  0xcb95777d0f8d2EfA5e836Cb65f814dF8C7261d83 \
  0 \
  $(cast calldata "createPair(address,uint256,uint256,bytes)" $TOKEN 1000 2000 $FEE_DATA)
...
MAINNET_DEX_OWNER="0x22C1522276855B028c31a731BA10D125811Af37c"
...
# Market Owners
GAME_USDTX_OWNER="0xafcc9E7d739b03CC53e2152d368f365430CF3CCa"
VERSE8_OWNER="0x4469a9879Ec02CE4313bef5e2F21b4F17f2B70e4"  # Verse8MarketOwner contract
VERSE8_ADMIN="0xafcc9E7d739b03CC53e2152d368f365430CF3CCa"   # DEFAULT_ADMIN_ROLE holder
```
...
TESTNET_DEX_OWNER="0x535c7b8C25eADACCAf857652cA34cF4c010957E2"

# Markets
GAME_MARKET="0x1d86752372281D0573Cb70319b434D73f2daFd59"
USDTX_MARKET="0x7D2fA2c51DBcF5AdaB5508B338f03853598c2352"
VERSE8_MARKET="0xCcb6F48f18da94B7d90e29adBF762FBf70564844"

# Market Owners
GAME_USDTX_OWNER="0x535c7b8C25eADACCAf857652cA34cF4c010957E2"
VERSE8_OWNER="0xFc2852EeaA01b4A9B52eF36Eb24DF56432f8043e"  # Verse8MarketOwner contract
VERSE8_ADMIN="0x535c7b8C25eADACCAf857652cA34cF4c010957E2"   # DEFAULT_ADMIN_ROLE holder
```
...
```bash
# 1. 변수 선언 (긴 주소만, 명확한 주석)

# 2. gorge write (첫 번째)

# 3. gorge write --append (추가 트랜잭션)

# 4. gorge sign --out <input>_sign.json <input>.json

# 5. gorge send --out <input>_send.json --rpc-url <RPC> --wait 60 <input>_sign.json
```
