...
import {ICrossDexV3} from "./interfaces/ICrossDex.sol";
...
contract PairImplV3 is IPairV2, IOwnable, UUPSUpgradeable, PausableUpgradeable {
    ...
    IMakerVaultFactory public MAKER_VAULT_FACTORY;
    ...
    function initialize(
        ...
        address _makerVaultFactory = ICrossDexV3(IMarketV3(MARKET).CROSS_DEX()).MAKER_VAULT_FACTORY();
        if (_makerVaultFactory == address(0)) revert PairInvalidInitializeData("makerVaultFactory");
        MAKER_VAULT_FACTORY = IMakerVaultFactory(_makerVaultFactory);
    }

    function reinitialize() external reinitializer(3) onlyOwner {
        address _makerVaultFactory = ICrossDexV3(IMarketV3(MARKET).CROSS_DEX()).MAKER_VAULT_FACTORY();
        if (_makerVaultFactory == address(0)) revert PairInvalidInitializeData("makerVaultFactory");
        MAKER_VAULT_FACTORY = IMakerVaultFactory(_makerVaultFactory);
    }
    ...
    function _exchangeSellOrder(uint256 orderId, address _owner, uint256 amount, address feeCollector, uint32 feeBps)
        ...
        returns (uint256)
    {
        address makerVault = MAKER_VAULT_FACTORY.ensureMakerVault(_owner);
        if (feeBps == 0) {
            QUOTE.safeTransfer(makerVault, amount);
            return 0;
        } else {
            ...
            QUOTE.safeTransfer(makerVault, value);
            return fee;
        }
    }
    ...
    function _exchangeBuyOrder(
        ...
        address makerVault = MAKER_VAULT_FACTORY.ensureMakerVault(_owner);
        BASE.safeTransfer(makerVault, buyBaseAmount);
