...
contract DEXV3UpgradeTest is DEXBaseTest {
    MakerVaultFactory public makerVaultFactory;

    address public crossDexImplV3;
    address public marketImplV3;
    address public pairImplV3;
    ...
    function setUp() external {
        ...
        // 2. deploy v2Logics
        crossDexImplV2 = address(new CrossDexImplV3());
        marketImplV2 = address(new MarketImplV2());
        pairImplV2 = address(new PairImplV3());

        // 3. upgrade CrossDexImpl
        vm.prank(OWNER);
        UUPSUpgradeable(address(CROSS_DEX)).upgradeToAndCall(
            crossDexImplV2, abi.encodeCall(CrossDexImplV3.reinitialize, (marketImplV2, pairImplV2, _makerVaultFactory))
        );
        ...
        // 5. upgrade PairImpl
        vm.prank(OWNER);
        UUPSUpgradeable(address(PAIR)).upgradeToAndCall(pairImplV2, abi.encodeCall(PairImplV3.reinitialize, ()));
    }

    function test_initial_status() external view {
        // check impl address
        CrossDexImplV3 crossDex = CrossDexImplV3(address(CROSS_DEX));
        assertEq(marketImplV2, crossDex.marketImpl(), "marketImpl");
        assertEq(pairImplV2, crossDex.pairImpl(), "pairImpl");
        assertEq(
            address(PairImplV3(address(PAIR)).makerVaultFactory()), address(makerVaultFactory), "makerVaultFactory"
        );
    }
